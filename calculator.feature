Feature: Calculator Operations

  Scenario: Add two positive numbers
    Given I have the numbers 2 and 3
    When I add them
    Then the result should be 5.0

  Scenario: Add a negative and positive number
    Given I have the numbers -1 and 1
    When I add them
    Then the result should be 0.0

  Scenario: Add two zeros
    Given I have the numbers 0 and 0
    When I add them
    Then the result should be 0.0

  Scenario: Subtract to get a positive result
    Given I have the numbers 10 and 4
    When I subtract them
    Then the result should be 6.0

  Scenario: Subtract to get a negative result
    Given I have the numbers 0 and 5
    When I subtract them
    Then the result should be -5.0

  Scenario: Multiply two positive numbers
    Given I have the numbers 3 and 4
    When I multiply them
    Then the result should be 12.0

  Scenario: Multiply a negative and positive number
    Given I have the numbers -2 and 5
    When I multiply them
    Then the result should be -10.0

  Scenario: Divide evenly
    Given I have the numbers 10 and 2
    When I divide them
    Then the result should be 5.0

  Scenario: Divide with a decimal result
    Given I have the numbers 7 and 2
    When I divide them
    Then the result should be 3.5

  Scenario: Divide by zero raises an error
    Given I have the numbers 5 and 0
    When I divide them
    Then a ValueError should be raised
